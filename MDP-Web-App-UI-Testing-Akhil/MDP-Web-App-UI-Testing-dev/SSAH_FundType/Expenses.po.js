var AddExpensesPage = function () {
    ...
    this.get = async function () {
        await browser.get('http://qa2.mydirectplan.com/app/#/login');
    };
    ...
    this.AddExpenseTab = async function (index) {
        await Add_expense_tab.isPresent();
        await Add_expense_tab.get(index).isPresent();

        console.log('element is exist');
        await Add_expense_tab.get(index).click(); // Side menu list//
    }
    ...
    this.Expense_list = async function (ind) {
        await Exp_list.get(ind).click(); //Expense list//
    }
    this.SupportWorker = async function (date1, date2, value1, Amount) { //SUPPORT WORKER ALL FIELDS CODE//

        await Total_Amount.sendKeys(Amount);
        await start_date.sendKeys(date1);
        await End_date.sendKeys(date2);
        await browser.sleep(5000)
        await support_worker_list.isPresent();
        console.log('select support worker list');
        await NameOf_Support_worker.click();
        await support_worker_list.get(value1).click();
    }
    this.SupportWorker_HourlyRate = function (date1, date2, value1, value2, SHhrs, HR) { //SUPPORT WORKER ALL FIELDS CODE//
